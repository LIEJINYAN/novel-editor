        ...
        <div className="p-4 overflow-y-auto max-h-[calc(80vh-120px)]">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-editor-bg rounded-lg p-3 text-center">
                ...
                <div className="text-xs text-editor-muted mt-1">字数</div>
              </div>
              <div className="bg-editor-bg rounded-lg p-3 text-center">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-editor-muted">
                {weekTotalWords} 字 / {formatTime(weekTotalTime)}
              </span>
            </div>
            <div className="bg-editor-bg rounded-lg p-3">
              <div className="flex items-end gap-1 h-20">
                {weekStats.map((stat) => (
                  ...
                  >
                    <div
                      className="w-full bg-editor-accent rounded-t"
                      style={{
                        height: `${Math.max((stat.wordsWritten / maxWeekWords) * 60, 2)}px`,
                      }}
                    />
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-editor-muted">
                {monthTotalWords} 字 / {formatTime(monthTotalTime)}
              </span>
            </div>
            <div className="bg-editor-bg rounded-lg p-3 space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="text-editor-muted">总字数</span>
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-editor-text font-medium">
                  {Math.round(monthTotalWords / 30)}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-editor-muted">日均时长</span>
                <span className="text-editor-text font-medium">
                  {formatTime(Math.round(monthTotalTime / 30))}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-editor-muted">写作天数</span>
                <span className="text-editor-text font-medium">
                  {monthStats.filter((s) => s.wordsWritten > 0).length}
                </span>
              </div>
            </div>
          </div>
